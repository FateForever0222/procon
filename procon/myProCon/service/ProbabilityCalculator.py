...
import math
from procon.myProCon.model import RankProbaa
...
class ProbabilityCalculator:
                    ...
                    for i in line:
                        ...
                        if i is not '\n':
                            if i is 'S':
                                ...
                                self.__rpaa[sindex][20].add()
                                self.__prob6aa[sindex][6] += 1.0  # qualified
                                self.__seq_tmp[self.__sequence_number - 1].append('-')
                            sindex += 1
                print("Probability Calculator inited successfully")
                f.close()
        except:
            self.__available = False
            print("出现错误")
    ...
    def get_min_gap(self):
        gap_list = [i[20] for i in self.get_gap_20()]
        return min(gap_list)

    def get_ave_gap(self):
        sum_gap = sum([i[20] for i in self.get_gap_20()])
        return sum_gap / len(self.get_gap_20())
